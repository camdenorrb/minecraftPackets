...
func main() {
	...
	file, err := os.ReadFile("palette/blocks1_20_4.json")
	if err != nil {
	...
	*/
	...
	printOutPaletteProperties("palette/blocks1_20_4.json")

	// Write proto file
	blockProto := blocks.AsProto()
	if blockProto.IsError() {
		panic(errorx.Panic(blockProto.UnwrapError()))
	}

	blockProtoValue := blockProto.Unwrap()

	blockProtoFile, err := os.Create("palette/blocks1_20_4.proto")
	if err != nil {
		panic(errorx.Panic(err))
	}
	defer blockProtoFile.Close()

	marshalled, err := proto.Marshal(&blockProtoValue)
	if err != nil {
		panic(errorx.Panic(err))
	}
